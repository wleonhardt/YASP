...
describe("RoomUtilityMenu", () => {
  ...
  it("opens from the utility trigger and closes on Escape while returning focus", async () => {
    ...
    const trigger = screen.getByRole("button", { name: /open room preferences/i });
    await user.click(trigger);

    expect(screen.getByRole("dialog", { name: /room preferences/i })).toBeInTheDocument();

    await user.keyboard("{Escape}");

    expect(screen.queryByRole("dialog", { name: /room preferences/i })).not.toBeInTheDocument();
  ...
  it.each([
    [
      "en",
      "Open room preferences — Connected",
      "Room preferences",
      "Session status",
      "Appearance",
      "Connected",
    ...
    [
      "es",
      "Abrir preferencias de la sala — Conectado",
    ...
    [
      "fr",
      "Ouvrir les préférences de la salle — Connecté",
    ...
    [
      "de",
      "Raumeinstellungen öffnen — Verbunden",
      "Raumeinstellungen",
      "Sitzungsstatus",
      "Darstellung",
      "Verbunden",
    ...
    [
      "pt",
      "Abrir as preferências da sala — Conectado",
      "Preferências da sala",
      "Estado da sessão",
      "Aspeto",
      "Conectado",
    ],
    ["ja", "ルーム設定を開く — 接続済み", "ルーム設定", "セッション状態", "表示", "接続済み"],
    ["ko", "룸 설정 열기 — 연결됨", "룸 설정", "세션 상태", "표시", "연결됨"],
    ["zh-Hans", "打开房间偏好 — 已连接", "房间偏好", "会话状态", "外观", "已连接"],
    ["zh-Hant", "開啟房間偏好 — 已連線", "房間偏好", "工作階段狀態", "外觀", "已連線"],
  ] as const)(
